...
const numberTheoryLinks: Array<[string,string[]]> = [
	["sums_and_products", [
		...
		"√2 + √3 is irrational%root2PlusRoot3IsIrr",
	]],
	["floor_and_ceil_functions", [
		"[x + n] = [x] + n where n ∈ Z%addingIntegers",
		"[x] + [x + 1/2] = [2x]%HermitesIdentitySpecialCase",
		"[2x] + [2y] ≥ [x] + [y] + [x+y] (incomplete)%floorAdd",
	...
	["divisibility", [
		...
		"there are [x/d] positive integers that are ≤ x (where x ∈ Z<sup>+</sup>) and divisible by d%divisibility5",
	...
	["prime_numbers", [
		...
		"a powerful number is a product of a square and a cube%powerful_from_square_and_cube",
		...
		"if p is prime and p|a<sup>n</sup> then p|a%primeLemma4",
	...
	["exactly_divides",[
		...
		"[p<sup>a</sup> ∥ m] ∧ [p<sup>b</sup> ∥ n] ⇒ [p<sup>a+b</sup> ∥ mn]%exactlyDividesLemma2",
		"[p<sup>a</sup> ∥ m] ⇒ [p<sup>an</sup> ∥ m^n]%exactlyDividesLemma3",
		"Legendre's formula",
	...
	["more_gcd", [
		...
		"(α, β) = 1 ∧ ε|(α+β) ⇒ (α, ε) = (β, ε) = 1%gcdLemma2",
		"(α, β) = δ ⇒ (α/δ, β/δ) = 1%gcdLemma8",
		"(m, β) = 1 ⇒ (mα, β) = (α, β)%gcdLemma3",
		"(α, β) = 1 ∧ αβ = c<sup>n</sup> ⇒ ∃x, y ∈ Z<sup>+</sup> such that  α = x<sup>n</sup> ∧ β = y<sup>n</sup>%gcdLemma6",
		"if α is odd, then gcd(α, α-2) = 1%gcdLemma10",
		"(α<sub>1</sub>, α<sub>2</sub>, α<sub>3</sub>, ..., α<sub>n</sub>) ⇒ ((α<sub>1</sub>, α<sub>2</sub>), α<sub>3</sub>, ..., α<sub>n</sub>)%gcdLemma7",
		"(α, β, ε) = 1 ⇒ (αβ, ε) = (α, ε)(β, ε)%gcdLemma4",
		"if (x, y) divides (x + y), then there are infinite values of x and y%gcdDividesPlus",
		"m * (α<sub>1</sub>, α<sub>2</sub>, ...) = (mα<sub>1</sub>, mα<sub>2</sub>, ...)%gcdLemma15",
		"(α, β) = δ ∧ (α, ε) = δ ⇒ (α, β, ε) = δ%gcdLemma17",
		"(α, β) = 1 ⇒ (α<sup>m</sup>, β<sup>n</sup>) = 1%gcdLemma12",
		"(α, β) = (α + kβ, β) where k is any integer%gcdLemma1",
		"(α, β) = 1 ⇒ (α+β, α-β) is ≤ 2%gcdLemma16",
		"(α, β) = 1 ⇒ (α<sup>2</sup>+β<sup>2</sup>, α+β) is ≤ 2%gcdLemma9",
		"(α, β) = 1 ⇒ (α + β, αβ) = 1%gcdLemma11",
		"[(a,b) = (c,d) = 1 ∧ a/b + c/d ∈ Z] ⇒ b = d%gcdLemma5",
		"6n-1, 6n+1, 6n+2, 6n+3 and 6n+5 are pairwise relatively prime%pairwiseRelative6n",
	...
	["lcm", [
		...
		"[a, b]|c ⟺ a|c and b|c%lcmLemma1",
		"a,b ∈ N<sup>+</sup> ⇒ [a, b] = cd where c|a, d|b and (c, d) = 1%lcmLemma4",
		"(a, b) = (a + b, [a, b]) where a, b ∈ Z<sup>+</sup>%lcmLemma5",
		"[(a, b), c] = ([a, c], [b, c]) and ([a, b], c) = [(a, c), (b, c)] where a, b, c ∈ Z<sup>+</sup>%lcmLemma6",
		"[a, b, c] = abc(a,b,c)/(a,b)(a,c)(b,c) where a, b, c ∈ Z<sup>+</sup>%lcmLemma7",
		"[(a, b), (a, c), (b, c)] = ([a, b], [a, c], [b, c]) where a, b, c ∈ Z<sup>+</sup>%lcmLemma8"
	]],
	["modular_arithmetic", [
