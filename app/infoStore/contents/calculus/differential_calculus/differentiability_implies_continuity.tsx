...
const content = [
  ['h1', 'Proof That If f(x) Is Differentiable At b, Then f Is Continuous At b'],
  ['pmain', 'If \\(f(x)\\) is differentiable at \\(b\\), then \\(f\'(b)\\) exists:'], 
  ["displayFormula","\\[f'(b) = \\lim_{x→b} \\frac{f(x) - f(b)}{x - b}\\]"],
  ['pmain', 'We want to show that \\(f(x)\\) is continuous at \\(b\\) by showing that \\(\\lim_{x→b} f(x) = f (b)\\). We can rewrite \\(\\lim_{x→b} f(x)\\) as:'], 
