...
const content = [
  ...
  ['displayimg', 'as6.png'],
  ['pmain', 'Keep in mind that \\(\\cos(y)\\) will always be positive since \\(y \\in [\\frac{-\\pi}{2} , \\frac{\\pi}{2}]\\), so \\(\\sqrt{1-x^2}\\) will also always be positive. Now let\'s find the derivative of \\(\\arcsin(\\frac{x}{a})\\). We can use the chain rule:'],
  ['displayFormula','\\[\\frac{d}{dx} \\arcsin \\left( \\frac{x}{a} \\right) = \\frac{1}{\\sqrt{1-\\frac{x^2}{a^2}}} \\frac{1}{a}\\]'],
  ['pmain', 'Since \\(\\cos(y)\\) is always positive, so:'],
